...
func getAssignmentsQueryStatement(input *domain.GetManyInput) squirrel.SelectBuilder {
	mainStmt := psql.
		Select(
			...
			"u.attrs->>'full_name'",
			"u.id",
			"(select sum((sheets.value ->> 'rows')::numeric) from jsonb_array_elements(app.attrs -> 'sheets') sheets)",
			"(select sum((sheets.value ->> 'expenses')::numeric) from jsonb_array_elements(app.attrs -> 'sheets') sheets)",
